...
class RserverRepoStack(core.Stack):
    ...
    def __init__(self, scope:core.Construct, construct_id: str, **kwargs):
        ...
        if construct_id == CdkConfig.config['hosted_zone_stack_name'] and \
            CdkConfig.hosted_zone_deploy == 'true':
            HostedZoneService.create_hosted_zone(stack)
        if construct_id == CdkConfig.config['primary_rserver_stack_name'] and \
            CdkConfig.primary_rserver_deploy == 'true' and CdkConfig.hosted_zone_deploy == 'false':
            ...
            ec2_securitygroup = NetworkService.create_ec2_security_group(stack, vpc, \
                alb_security_group)
            ec2server = EC2Service.create_ec2_service(stack, vpc, private_subnet, \
                ec2_securitygroup, iamrole)
            ...
            alb = AlbService.create_alb(stack, vpc, ec2server, public_subnet, alb_security_group,\
                 certificate.certificate_arn)              #pylint: disable=too-many-function-args
